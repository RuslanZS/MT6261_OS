...
typedef struct tag_IRQSOURCE
{
    int32_t SourceIdx;
} TIRQSOURCE, *pIRQSOURCE;
...
bool NVIC_RegisterIRQ(uint32_t SourceIdx, void (*Handler)(void), uint8_t Sense, bool Enable);
bool NVIC_UnregisterIRQ(uint32_t SourceIdx);
bool NVIC_RegisterEINT(uint32_t SourceIdx, void (*Handler)(void), uint8_t Sense, uint16_t Debounce, bool Enable);
bool NVIC_UnregisterEINT(uint32_t SourceIdx);
bool NVIC_EnableEINT(uint32_t SourceIdx);
bool NVIC_DisableEINT(uint32_t SourceIdx);
bool NVIC_EnableIRQ(uint32_t SourceIdx);
bool NVIC_DisableIRQ(uint32_t SourceIdx);